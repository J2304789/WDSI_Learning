...
Source_pair_secret=st.text_input("Enter your Secret Key:")
Source_pair_public= st.text_input("Enter your Public Key")
Destination_pair_public=st.text_input("Enter Destination Public Key:")
if Source_pair_secret and Source_pair_public and Destination_pair_public:
    Source_pubkey=Source_pair_public.public_key
    Destination_pubkey=Destination_pair_public.public_key
...
#checks if destination account exists,as if it doesn't, it will still be charged transaction fees
    ...
    #signs transaction with Source_pair's Private key in order to verify transaction's authorization
        Transaction_Trust.sign(Source_pair_secret.secret)
