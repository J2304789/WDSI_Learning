...
def app():
    ...
    queryResult = sc.SQL_execute_twoway_statement(query)[0][0]                                            #Because the function returns an array with tuples, we want the first registry and the first element from the tuple.
    ...
    if queryResult:                                                                     #If the account is already registered, display message and continue in loop.
        ...
        with st.sidebar.form(key='sigup_form'):
            sign_up_form(profileID, hashedProfileID, sc)

def sign_up_form(profileID, hashedProfileID, sc):
    #REPLACE WITH STREAMLIT INPUT 
